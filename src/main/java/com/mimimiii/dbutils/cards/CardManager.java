...
public class CardManager {
    ...
    public boolean createPackage(List<Card> cards){
        ...
        try {
            ...
            for (Card card: cards){
                PreparedStatement ps = conn.prepareStatement("SELECT COUNT(id) FROM cards WHERE id = ? AND collection IS NULL;");
    ...
    public String showUserCards (User user){
        try {
            Connection conn = DatabaseService.getInstance().getConnection();
            PreparedStatement ps = conn.prepareStatement("SELECT id, name, damage FROM cards WHERE owner = ?;");
    ...
    public String showUserDeck (User user){
        try {
            Connection conn = DatabaseService.getInstance().getConnection();
                                                                                                // sql im cards - in user überneh?
            PreparedStatement ps = conn.prepareStatement("SELECT id, name, damage FROM cards WHERE owner = ? AND collection = 'in_deck';");
            ps.setString(1,user.getUsername());
            String json = result2Json(ps.executeQuery());
            ps.close();
            conn.close();
    ...
    public boolean acquirePackage2User(User user){
        try {
            ...
            // Update Cards Owner and Collection                                    // stack updaten !
            PreparedStatement ps_Card = conn.prepareStatement("UPDATE cards SET owner = ?, collection = 'stack' WHERE id = ?;");
            for (int i = 2; i < 7; i++){
            ...
            ps_Card.close();
            // Delete Package
            PreparedStatement ps_Package = conn.prepareStatement("DELETE FROM packages WHERE id = ?;");
